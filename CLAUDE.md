...
**Brain files live in `docs/brain/` but are NOT auto-loaded (too large for context). Read them on-demand when needed.**

**After EVERY coding task that edits source files (tsx, ts, css, sql), you MUST:**
1. Read then update `docs/brain/feature-map.md` with the FULL behavioral spec of what changed — what each button does to UI, backend, and other pages
2. Read then update `docs/brain/intent-log.md` with WHY the change was made
3. Read then update `docs/brain/test-matrix.md` with new test cases for the changed features
...
**The feature map is the source of truth for what every button SHOULD do.** When writing a new feature, first read its current state in the feature map, write its behavioral spec, THEN build it. When testing, read the test matrix to know exactly what to verify.
