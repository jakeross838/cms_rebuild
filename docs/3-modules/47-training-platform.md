...
1. **Training content becomes outdated after platform updates.** When the platform UI or workflow changes in a release, existing training videos, guided tours, and KB articles may show outdated screens, wrong button locations, or deprecated workflows. The system must maintain a mapping between training content and platform features/versions. When a release changes a feature that has associated training content, the system must auto-flag that content as "review needed" and notify the content team. Outdated content must display a visible warning banner ("This content was created for version X. The current version may look different.") until it is updated. Guided tour step selectors that no longer match DOM elements must gracefully skip the broken step rather than crash the tour. Training content freshness metrics (% of content up-to-date with current release) must be tracked on the admin dashboard.

2. **User fails certification exam multiple times.** When a user fails a certification assessment repeatedly (2+ attempts), the system must provide constructive guidance rather than just a failing score. After each failed attempt, the system must show: which topic areas had incorrect answers (without revealing exact questions and answers, to prevent memorization), recommended training content to review for each weak area, and a link to the relevant practice exercises in the sandbox. The 7-day waiting period between retake attempts should be enforced to encourage actual study. After 3 consecutive failures, the system should suggest scheduling an instructor-led session (paid add-on) or reaching out to support for personalized guidance. The system must also track repeated failures as a signal in the customer success dashboard -- a user struggling with certification may need proactive outreach.

3. **Training platform integrated with support and onboarding.** The training platform should not exist in isolation -- it must be tightly connected to the support system (Module 46) and the onboarding wizard (Module 41). When a support ticket is resolved, the agent should be able to attach a relevant training video or article to the resolution ("To prevent this in the future, watch this 3-minute video"). When a user completes the onboarding wizard, they should be automatically enrolled in the appropriate role-based training path. When a user submits a support ticket about a workflow they have not yet completed training for, the system should suggest the relevant training content before creating the ticket. Training completion data should be visible to support agents so they can tailor their assistance level.
