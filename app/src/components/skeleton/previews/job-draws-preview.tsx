      ...
      >
        <td className="py-3 px-4">
          <div className="flex items-center gap-2">
            ...
            {draw.aiNote && <Sparkles className="h-4 w-4 text-amber-500" />}
...
export function JobDrawsPreview() {
  ...
  const paidDraws = mockDraws.filter(d => ['paid', 'disbursed'].includes(d.status))
  const amountBilled = mockDraws
    .filter(d => !['draft'].includes(d.status))
    .reduce((sum, d) => sum + d.amount, 0)
  const amountPaid = paidDraws.reduce((sum, d) => sum + (d.disbursedAmount || 0), 0)
      ...
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-t border-amber-200 px-4 py-3">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-amber-700">
            Draw #4 pending bank inspection (inspector: Tom Williams). Draw #5 auto-generated from approved invoices -
            3 SOV lines need manual % verification. 2 lien waivers missing for Draw #4 package (XYZ Electric, Cool Air HVAC).
            Retainage reduced to 5% for draws 6-7 per contract milestone. At current pace, all draws complete on schedule.
          </p>
        </div>
      </div>
    </div>
