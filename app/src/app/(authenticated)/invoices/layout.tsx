...
export default function InvoicesLayout({
  ...
  const currentJobId = searchParams.get('job') ?? undefined
  ...
  // Build href preserving current path but changing ?job= param
  ...
  // Don't show sidebar on sub-pages like /invoices/extractions, /invoices/upload, /invoices/[id]
  const isListPage = pathname === '/invoices'
  const showSidebar = isListPage
  ...
  return (
    <div className="flex flex-1 overflow-hidden -m-4 md:-m-6">
      ...
      <aside className="hidden md:flex w-64 bg-card border-r border-border/40 flex-col overflow-hidden flex-shrink-0">
        {/* Header */}
        <div className="p-3 border-b border-border/40">
          <div className="flex items-center gap-2 mb-2">
            <Receipt className="h-4 w-4 text-muted-foreground" />
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Filter by Job</span>
          </div>
        ...
        <Link
          href="/invoices"
          className={cn(
            'flex items-center gap-2 px-3 py-2.5 border-b border-border/40 transition-colors border-l-2',
            !currentJobId
        ...
        >
          ...
          <span className={cn(
            'text-sm',
            !currentJobId ? 'font-semibold text-foreground' : 'font-medium text-foreground'
          )}>
            All Jobs
          </span>
        </Link>
