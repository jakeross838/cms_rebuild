  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative max-w-md">
        ...
        <form><Input type="search" name="search" placeholder="Search dashboards..." aria-label="Search dashboards" defaultValue={params.search} className="pl-10" /></form>
      </div>
        ...
        <CardContent>
          {reports.length > 0 ? (
            <div className="divide-y divide-border">
              {reports.map((report) => (
                <Link key={report.id} href={`/dashboards/${report.id}`} className="block py-3 first:pt-0 last:pb-0 hover:bg-accent/50 -mx-2 px-2 rounded-md transition-colors">
                  <div className="flex items-center justify-between">
                    <div>
