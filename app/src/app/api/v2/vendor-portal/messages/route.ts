...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    let query = supabase
      ...
      .select('id, company_id, vendor_id, job_id, subject, body, direction, sender_id, is_read, read_at, attachments, parent_message_id, created_at, updated_at', { count: 'exact' })
    ...
    if (filters.q) {
      query = query.or(`subject.ilike.${safeOrIlike(filters.q)},body.ilike.${safeOrIlike(filters.q)}`)
    }
...
export const POST = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'vendor_portal_message.create' }
)
