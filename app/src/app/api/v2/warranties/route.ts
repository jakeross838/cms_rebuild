...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    let query = supabase
      .from('warranties')
      .select('id, company_id, job_id, title, description, warranty_type, status, vendor_id, start_date, end_date, coverage_details, exclusions, document_id, contact_name, contact_phone, contact_email, transferred_to, transferred_at, created_by, created_at, updated_at', { count: 'exact' })
    ...
    if (filters.q) {
      query = query.or(`title.ilike.${safeOrIlike(filters.q)},contact_name.ilike.${safeOrIlike(filters.q)}`)
    }
