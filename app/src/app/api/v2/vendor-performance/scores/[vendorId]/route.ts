...
export const GET = createApiHandler(
  async (req: NextRequest, ctx: ApiContext) => {
    ...
    const { data, error } = await supabase
      .from('vendor_scores')
      .select('id, company_id, vendor_id, quality_score, timeliness_score, communication_score, budget_adherence_score, safety_score, overall_score, data_point_count, calculation_window_months, manual_adjustment, manual_adjustment_reason, calculated_at, created_by, created_at, updated_at')
...
export const PUT = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'vendor_performance_score.update' }
)
