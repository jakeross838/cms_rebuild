...
export const POST = createApiHandler(
  async (req: NextRequest, ctx: ApiContext) => {
    ...
    if (openEntryError && openEntryError.code !== 'PGRST116') {
      const mapped = mapDbError(openEntryError)
      return NextResponse.json(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm', 'superintendent', 'office', 'field'], auditAction: 'time_entry.clock_in' }
)
