...
export const PUT = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm', 'office'], auditAction: 'reports_schedule.update' }
)
...
export const DELETE = createApiHandler(
  async (req: NextRequest, ctx: ApiContext) => {
    ...
    if (!existing) {
      return NextResponse.json(
        { error: 'Not Found', message: 'Report schedule not found', requestId: ctx.requestId },
        { status: 404 }
      )
    }
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm', 'office'], auditAction: 'reports_schedule.delete' }
)
