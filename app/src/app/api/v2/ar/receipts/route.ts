...
export const POST = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    const { data: applications, error: appError } = await supabase
      ...
      .insert(appRecords)
      .select('id, receipt_id, invoice_id, amount, created_at')
    ...
    const rpcResults = await Promise.all(
      input.applications.map((app) =>
        supabase.rpc('apply_receipt_to_invoice', {
  ...
  { requireAuth: true, requiredRoles: ['owner', 'admin', 'pm', 'office'], rateLimit: 'financial', auditAction: 'receipt.create' }
)
