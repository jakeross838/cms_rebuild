...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    let query = supabase
      .from('ar_invoices')
      .select('id, company_id, client_id, job_id, invoice_number, invoice_date, due_date, amount, balance_due, status, terms, notes, created_by, created_at, updated_at', { count: 'exact' })
    ...
    if (filters.q) {
      query = query.or(`invoice_number.ilike.${safeOrIlike(filters.q)},notes.ilike.${safeOrIlike(filters.q)}`)
    }
...
export const POST = createApiHandler(
  ...
  { requireAuth: true, requiredRoles: ['owner', 'admin', 'pm', 'office'], rateLimit: 'financial', auditAction: 'invoice.create' }
)
