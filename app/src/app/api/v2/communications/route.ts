 ...
 *
 * GET  /api/v2/communications — List communications (filtered by company)
 * POST /api/v2/communications — Create a new communication
 */
...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    const parseResult = listCommunicationsSchema.safeParse({
      page: url.searchParams.get('page') ?? undefined,
      limit: url.searchParams.get('limit') ?? undefined,
      job_id: url.searchParams.get('job_id') ?? undefined,
      communication_type: url.searchParams.get('communication_type') ?? undefined,
    ...
    let query = supabase
      .from('communications')
      .select('id, company_id, job_id, subject, message_body, communication_type, priority, recipient, status, notes, created_by, created_at, updated_at', { count: 'exact' })
    ...
    if (filters.q) {
      query = query.or(`subject.ilike.${safeOrIlike(filters.q)},message_body.ilike.${safeOrIlike(filters.q)}`)
    }
...
// ============================================================================
// POST /api/v2/communications — Create communication
// ============================================================================
...
export const POST = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'communication.create' }
)
