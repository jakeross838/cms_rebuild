...
export const GET = createApiHandler(
  async (req: NextRequest, ctx: ApiContext) => {
    ...
    const { data, error } = await supabase
      .from('rfis')
      .select('id, company_id, job_id, rfi_number, subject, question, status, priority, category, assigned_to, due_date, cost_impact, schedule_impact_days, related_document_id, created_by, answered_at, closed_at, closed_by, created_at, updated_at, rfi_responses(id), rfi_routing(id, rfi_id, company_id, routed_to, routed_by, routed_at, status, notes, created_at, updated_at)')
      .eq('id', id)
...
export const PUT = createApiHandler(
  async (req: NextRequest, ctx: ApiContext) => {
    ...
    const { data, error } = await supabase
      ...
      .eq('company_id', ctx.companyId!)
      .is('deleted_at', null)
      .select('id, company_id, job_id, rfi_number, subject, question, status, priority, category, assigned_to, due_date, cost_impact, schedule_impact_days, related_document_id, created_by, answered_at, closed_at, closed_by, created_at, updated_at')
      .single()
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'rfi.update' }
)
...
export const DELETE = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'rfi.archive' }
)
