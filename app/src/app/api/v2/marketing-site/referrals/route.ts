...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    if (filters.q) {
      query = query.or(`referred_email.ilike.${safeOrIlike(filters.q)},referred_company_name.ilike.${safeOrIlike(filters.q)},referral_code.ilike.${safeOrIlike(filters.q)}`)
    }
...
export const POST = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin'], auditAction: 'marketing_site_referral.create' }
)
