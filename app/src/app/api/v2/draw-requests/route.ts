...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    if (filters.q) {
      query = query.or(`lender_reference.ilike.${safeOrIlike(filters.q)},notes.ilike.${safeOrIlike(filters.q)}`)
    }
...
export const POST = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    // Record creation in history (non-blocking)
  ...
  { requireAuth: true, requiredRoles: ['owner', 'admin', 'pm', 'office'], rateLimit: 'financial', auditAction: 'draw_request.create' }
)
