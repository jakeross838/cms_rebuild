...
export const GET = createApiHandler(
  async (req: NextRequest, ctx: ApiContext) => {
    ...
    const { data, error } = await supabase
      .from('bid_packages')
      .select('id, company_id, job_id, title, description, trade, scope_of_work, bid_due_date, status, documents, created_by, created_at, updated_at, bid_invitations(id), bid_responses(id), bid_awards(id)')
      .eq('id', id)
...
export const PUT = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'bid_package.update' }
)
...
export const DELETE = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'bid_package.archive' }
)
