...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    let query = supabase
      ...
      .select('id, company_id, job_id, client_user_id, payment_number, amount, payment_method, status, reference_number, description, draw_request_id, invoice_id, payment_date, notes, created_by, created_at, updated_at', { count: 'exact' })
    ...
    if (filters.q) {
      query = query.or(`payment_number.ilike.${safeOrIlike(filters.q)},description.ilike.${safeOrIlike(filters.q)},reference_number.ilike.${safeOrIlike(filters.q)}`)
    }
...
export const POST = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'client_portal_payment.create' }
)
