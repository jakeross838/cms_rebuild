...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    let query = supabase
      ...
      .select('id, company_id, job_id, name, room, sort_order, pricing_model, allowance_amount, deadline, lead_time_buffer_days, assigned_to, status, designer_access, notes, created_by, created_at, updated_at', { count: 'exact' })
    ...
    if (filters.q) {
      query = query.or(`name.ilike.${safeOrIlike(filters.q)},room.ilike.${safeOrIlike(filters.q)}`)
    }
...
export const POST = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    const { data, error } = await supabase
      ...
      .select('id, company_id, job_id, name, room, sort_order, pricing_model, allowance_amount, deadline, lead_time_buffer_days, assigned_to, status, designer_access, notes, created_by, created_at, updated_at')
      .single()
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm'], auditAction: 'selections_category.create' }
)
