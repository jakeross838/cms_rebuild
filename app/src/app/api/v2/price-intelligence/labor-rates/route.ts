...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    if (filters.q) {
      query = query.or(`trade.ilike.${safeOrIlike(filters.q)},region.ilike.${safeOrIlike(filters.q)},notes.ilike.${safeOrIlike(filters.q)}`)
    }
...
export const POST = createApiHandler(
  ...
  { requireAuth: true, rateLimit: 'api', requiredRoles: ['owner', 'admin', 'pm', 'office'], auditAction: 'price_intelligence_labor_rate.create' }
)
