...
export const PATCH = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    const { data: updated, error: updateError } = await typedUpdate(supabase, 'schedule_tasks', updateData)
      .eq('id', targetId)
      .eq('company_id', ctx.companyId!)
      .select()
      .single() as { data: ScheduleTask | null; error: { message: string } | null }
...
export const DELETE = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    const { data: deleted, error } = await typedUpdate(supabase, 'schedule_tasks', { deleted_at: now, updated_at: now })
      .eq('id', targetId)
      .eq('company_id', ctx.companyId!)
