/**
 * Daily Logs API — Get, Update & Delete
 *
 * GET    /api/v1/daily-logs/[id] — Get a single daily log with entries
 * PATCH  /api/v1/daily-logs/[id] — Update a daily log
 * DELETE /api/v1/daily-logs/[id] — Soft-delete a daily log
 */
...
export const GET = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    const { data: log, error } = await supabase
      ...
      .is('deleted_at', null)
      .single() as { data: DailyLog | null; error: { message: string } | null }

    if (error || !log) {
      logger.warn('Daily log not found', { targetId })
      return NextResponse.json(
...
export const PATCH = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    // Verify exists and is still in draft
    ...
    if (existing.status !== 'draft') {
      return NextResponse.json(
        { error: 'Conflict', message: 'Only draft logs can be edited', requestId: ctx.requestId },
        { status: 409 }
      )
    }
    ...
    logger.info('Daily log updated', { targetId, companyId: ctx.companyId! })
...
export const DELETE = createApiHandler(
  async (req, ctx: ApiContext) => {
    ...
    logger.info('Daily log soft-deleted', { targetId, companyId: ctx.companyId! })
