...
- **Why** — Module 52 provides warehouse/job site inventory management, receiving, transfers, consumption tracking, and material request workflows for construction companies. Builders need to track materials across multiple locations (warehouses, job sites, vehicles), record movement of materials (receiving from vendors, transferring between sites, consuming on jobs, adjusting for counts, returning to vendors), and enable field crews to request materials from the office/warehouse. This is a Phase 3 module that integrates with purchase orders (Module 18), cost tracking (Module 09), and vendor management (Module 10). V1 builds the core data layer and CRUD APIs for the item catalog, locations, stock levels, transaction log, and material request workflow.
- **What** — 1 migration (6 tables: inventory_items, inventory_locations, inventory_stock, inventory_transactions, material_requests, material_request_items), 1 types file (4 type unions, 6 interfaces, 4 constant arrays), 1 Zod schemas file (4 enums, 14 CRUD schemas), 9 API route files (17 endpoint handlers), 44 acceptance tests.
- **How** — inventory_items is the master catalog with SKU, category, unit of measure, unit cost, and reorder points. inventory_locations supports 4 types (warehouse, job_site, vehicle, other) and can link to a job_id. inventory_stock tracks per-item-per-location levels with UNIQUE constraint. inventory_transactions is an append-only log capturing all material movements with from/to locations, reference linking (e.g., to purchase orders), job/cost-code attribution, and auto-calculated total_cost. material_requests use a draft->submitted->approved->fulfilled lifecycle with line items that can reference catalog items or free-text descriptions. Approval endpoint enforces submitted-only constraint and records approver/timestamp.
- **Rules** — (1) Multi-tenant: all 6 tables have company_id with RLS. (2) Soft delete on inventory_items and material_requests via deleted_at. (3) Locations use is_active=false for deactivation. (4) Stock has UNIQUE(item_id, location_id) constraint. (5) Transactions are append-only (no update/delete). (6) Transaction total_cost auto-calculated from unit_cost * quantity when not provided. (7) Material requests only editable in draft/submitted status. (8) Material requests only deletable in draft status. (9) Material requests only approvable from submitted status. (10) Material request items use ON DELETE CASCADE from parent. (11) Quantities use numeric(10,3) for fractional units. (12) GIN trigram index on inventory_items.name for fast text search.
- **Connected to** — Module 01 (auth required on all endpoints), Module 03 (jobs table FK on locations/transactions/requests, cost_codes FK on transactions), Module 09 (budget/cost tracking -- consumption transactions can flow as actual costs), Module 10 (vendor management -- future: vendor FK on items/transactions), Module 18 (purchase orders -- transactions reference POs for receiving), Module 23 (price intelligence -- unit_cost data feeds price comparison)

---

## Module 14 — Lien Waiver Management V1 Foundation (2026-02-24)

- **Why** — Module 14 spec requires creation, tracking, collection, and compliance enforcement of lien waivers from subcontractors and suppliers. Lien waivers are legally required documents that protect builders from mechanic's liens. The system must support 4 waiver types (conditional/unconditional x progress/final), track waiver status through a lifecycle (draft -> pending -> sent -> received -> approved), maintain state-specific templates, and provide compliance tracking per vendor per payment period. This is a Phase 3 (Financial Power) module tightly integrated with payments (Module 11) and draw requests (Module 15). V1 builds the core data layer, CRUD APIs, approval workflow, templates, and compliance tracking infrastructure.
- **What** — 1 migration (3 tables: lien_waivers, lien_waiver_templates, lien_waiver_tracking), 1 types file (2 type unions, 3 interfaces, 2 constant arrays), 1 Zod schemas file (2 enums, 10 CRUD schemas), 6 API route files (11 endpoint handlers), 32 acceptance tests. Spec maps `builder_id` to `company_id` for multi-tenant consistency.
- **How** — Migration follows existing pattern (CREATE TABLE IF NOT EXISTS, RLS on all 3 tables, indexes on company_id + FKs + status + type). Lien waivers use soft delete via deleted_at. Templates use hard delete (they're reusable config, not audit data). Tracking records link a vendor+job+period to an expected amount and compliance status. Approval workflow: only waivers in "received" status can be approved (403 otherwise). Updates blocked on approved/rejected waivers (403). All API routes use createApiHandler with requireAuth:true and rateLimit:'api'.
- **Rules** — (1) Multi-tenant: all 3 tables have company_id with RLS. (2) 4 waiver types enforced at DB CHECK and Zod schema level. (3) 6 statuses with lifecycle: draft -> pending -> sent -> received -> approved/rejected. (4) Soft delete only on lien_waivers (deleted_at). (5) Cannot update approved or rejected waivers (403). (6) Only received waivers can be approved (403). (7) Approval sets approved_by and approved_at. (8) Templates support state-specific forms via state_code. (9) Tracking records support compliance auditing with is_compliant boolean. (10) All monetary values numeric(15,2).
- **Connected to** — Module 01 (auth required on all endpoints), Module 03 (jobs table FK), Module 10 (vendors table FK), Module 11 (payment linkage — payment_id on waivers, future payment hold enforcement), Module 15 (draw requests — waiver package assembly), Module 06 (document storage — document_id on waivers)

---

## Module 15 — Draw Requests V1 Foundation (2026-02-24)

- **Why** — Module 15 spec requires draw request (payment application) generation, management, and tracking for construction financing. Builders need to create AIA G702/G703 format draws from a schedule of values, submit them through an internal approval workflow, and track submission to lenders. Draw requests are the primary mechanism for builders to receive payment from construction lenders. V1 builds the core data layer, CRUD APIs, and basic workflow transitions (draft -> pending_review -> approved). Advanced features like PDF generation, auto-generation from invoices, supporting document assembly, lender-specific templates, reconciliation, and multi-lender support are deferred to V2.
- **What** — 1 migration (3 tables: draw_requests, draw_request_lines, draw_request_history), 1 types file (2 type unions, 3 interfaces, 2 constant arrays), 1 Zod schemas file (2 enums, 10 CRUD/workflow schemas), 5 API route files (8 endpoint handlers), 36 acceptance tests. Spec maps `builder_id` to `company_id` for multi-tenant consistency.
- **How** — draw_requests uses AIA G702 summary fields (contract_amount, total_completed, retainage_pct/amount, total_earned, less_previous, current_due, balance_to_finish). draw_request_lines uses G703 continuation sheet columns (scheduled_value, previous_applications, current_work, materials_stored, total_completed, pct_complete, balance_to_finish, retainage). Line creation auto-calculates total_completed, pct_complete, and balance_to_finish from input values. Adding lines recalculates draw-level totals including retainage. Status transitions enforce business rules: only draft can be submitted (requires lines), only pending_review can be approved, only draft can be deleted. History table provides full audit trail. Soft delete on draw_requests via deleted_at.
- **Rules** — (1) All monetary values numeric(15,2). (2) Retainage_pct defaults to 10%, range 0-100. (3) pct_complete range 0-100 enforced at DB level (CHECK). (4) Only draft/rejected draws can be updated or have lines added. (5) Only draft draws can be soft-deleted. (6) Submit requires at least 1 line item. (7) Submit sets submitted_by/submitted_at. (8) Approve sets approved_by/approved_at. (9) All status transitions record history with action + details + performed_by. (10) Child tables (lines, history) cascade delete with parent. (11) No independent RLS on child tables.
- **Connected to** — Module 01 (auth required on all endpoints), Module 03 (jobs table FK), Module 03 (cost_codes FK on lines), Module 09 (budget/cost tracking — SOV can import from budget), Module 11 (accounting — draw amounts flow to AR), Module 14 (lien waivers — required for draw package), Module 17 (change orders — modify SOV amounts)

---

## Module 13 — AI Invoice Processing V1 Foundation (2026-02-24)

- **Why** — Module 13 spec requires AI-powered invoice processing that automates data extraction from uploaded invoice documents using OCR and machine learning. The system auto-codes invoices based on vendor history, provides confidence scoring on all extracted fields, and learns from human corrections. This module extends Module 11 (Native Accounting GL/AP/AR) -- all AI outputs feed into the same AP bill pipeline, approval workflow, and cost coding engine. V1 builds the core data layer for the extraction pipeline: extraction records, line item parsing, field mappings, automation rules, and an immutable audit trail. Defers to later phases: actual OCR/AI integration, document classification, batch processing UI, anomaly detection, vendor pattern learning, cross-tenant learning, email forwarding ingestion, threshold simulator, and review queue UI.
- **What** — 1 migration (5 tables: invoice_extractions, extraction_field_mappings, invoice_line_extractions, extraction_rules, extraction_audit_log), 1 types file (5 type unions, 5 interfaces, 7 constant arrays including CONFIDENCE_THRESHOLDS), 1 Zod schemas file (5 enums, 12 CRUD schemas), 6 API route files (10 endpoint handlers), 47 acceptance tests. Spec maps `builder_id` to `company_id` for multi-tenant consistency.
- **How** — Migration follows existing pattern (CREATE TABLE IF NOT EXISTS, RLS on tenant-level tables, cascade delete on child tables). Extraction pipeline lifecycle: pending -> processing -> completed/failed -> needs_review -> approved/rejected. Review endpoint validates reviewable status before allowing approve/reject. Create-bill endpoint converts completed extraction into AP bill (draft status) and links via matched_bill_id (prevents duplicate bill creation). Extraction rules support 4 types (field_mapping, auto_code, auto_approve, skip_review) with JSONB conditions/actions for flexible rule engines. Audit log is append-only with 8 action types tracking full extraction lifecycle.
- **Rules** — (1) Multi-tenant: all tenant tables have company_id with RLS. (2) Child tables (invoice_line_extractions, extraction_audit_log) use cascade delete from parent, no independent RLS. (3) Confidence scores 0-100 enforced at DB level (CHECK) and schema level. (4) Only pending/completed/needs_review extractions can be reviewed (409 on wrong status). (5) Only completed extractions can create bills (409 on wrong status). (6) Cannot create duplicate bills from same extraction (409 if matched_bill_id already set). (7) Extraction rules soft-deleted via is_active=false (never hard delete). (8) Priority on rules for ordering (higher = first evaluated). (9) Spec's CONFIDENCE_THRESHOLDS: AUTO_APPROVE=95, HUMAN_REVIEW=80, NEEDS_ATTENTION=70, REJECT=50.
- **Connected to** — Module 01 (auth required on all endpoints), Module 06 (document_id references uploaded documents), Module 11 (AP bills created from extractions flow into AP pipeline), Module 09 (cost_code_match_id on line items for budget tracking), Module 10 (vendor_match_id for vendor matching), Module 24 (AI Document Processing extends general document classification)
